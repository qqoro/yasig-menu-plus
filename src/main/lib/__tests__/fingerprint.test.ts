...
describe("computeFingerprint", () => {
  ...
  it("url 파일로 fingerprint 계산", async () => {
  ...
  it("실행 파일 없어도 일반 파일로 fingerprint 계산", async () => {
  ...
  it("exe + lnk 혼합 시 일관된 fingerprint", async () => {
    writeFileSync(join(TEST_DIR, "game.exe"), "exe");
    writeFileSync(join(TEST_DIR, "shortcut.lnk"), "lnk");
    const fp1 = await computeFingerprint(TEST_DIR, false);
    const fp2 = await computeFingerprint(TEST_DIR, false);
    expect(fp1).toBe(fp2);
  });

  it("하위 디렉토리 파일 포함하여 fingerprint 계산", async () => {
    writeFileSync(join(TEST_DIR, "game.exe"), "exe-content");
  ...
  it("비exe 파일이 다르면 다른 fingerprint", async () => {
    writeFileSync(join(TEST_DIR, "game.exe"), "exe-content");
  ...
  it("단일 파일(압축) fingerprint 계산", async () => {
    const filePath = join(TEST_DIR, "game.zip");
    writeFileSync(filePath, "zip-content");
    const fp = await computeFingerprint(filePath, true);
    expect(fp).toBeTypeOf("string");
    expect(fp).toHaveLength(64);
  });
});
