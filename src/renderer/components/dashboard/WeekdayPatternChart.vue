...
const props = defineProps<{
  data: DashboardStats["weekdayPattern"];
  theme?: string;
}>();
...
const option = computed(() => {
  const dayMap = new Map(props.data.map((d) => [d.weekday, d]));
  const days = Array.from({ length: 7 }, (_, i) => i);
  const values = days.map((d) => dayMap.get(d)?.totalSeconds ?? 0);
  const sessions = days.map((d) => dayMap.get(d)?.sessionCount ?? 0);
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        const d = params[0];
        return `${d.name}요일<br/>플레이 시간: ${formatHours(d.value)}<br/>세션 수: ${sessions[d.dataIndex]}회`;
      },
    },
...
<template>
  <Card>
    <CardHeader>
      <CardTitle class="text-sm font-medium">요일별 플레이 패턴</CardTitle>
    </CardHeader>
    <CardContent>
      <div
        v-if="data.length === 0"
