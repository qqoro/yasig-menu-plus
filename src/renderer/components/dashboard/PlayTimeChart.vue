...
const props = defineProps<{
  data: DashboardStats["monthlyPlayTime"];
  theme?: string;
}>();
...
const option = computed(() => ({
  ...
  tooltip: {
    ...
    formatter: (params: any) => {
      const d = params[0];
      return `${d.name}<br/>플레이 시간: ${formatHours(d.value)}<br/>세션 수: ${props.data[d.dataIndex]?.sessionCount ?? 0}회`;
    },
  },
  ...
  series: [
    {
      type: "bar",
      data: props.data.map((d) => d.totalSeconds),
...
<template>
  <Card>
    <CardHeader>
      <CardTitle class="text-sm font-medium">월별 플레이 추이</CardTitle>
    </CardHeader>
    <CardContent>
      <div
        v-if="data.length === 0"
